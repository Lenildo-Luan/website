...
```mermaid
flowchart TD
    A([Montagem do Componente]) --> B[useEffect roda uma vez]

    B --> C[Inicializa mouseX/Y\nno centro da tela]
    B --> D[addEventListener\nmousemove]
    B --> E[requestAnimationFrame\nanimate]

    C --> E

    D -->|a cada movimento| F[Atualiza\nmouseX / mouseY]

    E --> G[Lerp: currentX += mouseX - currentX × 0.1\ncurrentY += mouseY - currentY × 0.1]
    G --> H[Aplica translate3d\nno elemento DOM]
    H --> I[requestAnimationFrame\nanimate]
    I --> G

    F -.->|variáveis compartilhadas| G

    B --> J([Desmontagem do Componente])
